...
class UserService {
    ...
    signIn = async (req: Request, res: Response, next: NextFunction) => {
        const {email,password,FCMToken} : ISignInType = req.body
        ...
        if (FCMToken) {
            await this._redisCkient.addFCM({userId:user._id,FCMToken})
            const tokens = await this._redisCkient.getFCMs({userId:user._id})
            await this._notificationClient.sendNotifications({
                tokens,
                data:{title:`Welcome back ${user.userName}`,
                    body:`new logged at ${new Date()}`} 
            })  
        }
